...
"use strict"; {
    ...
    window.RuntimeInterface = class f {
        ...
        async CreateWorker(c, h, k) {
            ...
            if ("cordova" === this._exportType && this._isFileProtocol) return c = await this.CordovaFetchLocalFileAsArrayBuffer(k.isC3MainWorker ?
                c : this._scriptFolder + c), c = new Blob([c], {
                type: "application/javascript"
            }), new Worker(URL.createObjectURL(c), k);
            c = new URL(c, h);
        ...
        async _InitWorker(c, h) {
            ...
            let l = c.workerDependencyScripts || [],
                q = c.engineScripts;
            ...
            if ("cordova" === this._exportType)
                for (let p = 0, r = c.projectScripts.length; p < r; ++p) {
                    const w = c.projectScripts[p],
                        B = w[0];
                    if (B === c.mainProjectScript ||
        ...
        async _InitDOM(c, h) {
            ...
            this._domHandlers = A.map(p => new p(this));
            this._FindRuntimeDOMHandler();
            var k = c.engineScripts.map(p => "string" === typeof p ? (new URL(p,
            ...
            await Promise.all(k.map(p => b(p)));
            k = self.C3_ProjectScriptsStatus;
            const l = c.mainProjectScript,
                q = c.projectScripts;
            for (let [p, r] of q)
                if (r || (r = p), p === l) try {
                    r = await this._MaybeGetCordovaScriptURL(r), await b(r), "preview" !== this._exportType || k[p] || this._ReportProjectMainScriptError(p, "main script did not run to completion")
                } catch (w) {
                    this._ReportProjectMainScriptError(p,
                        w)
                } else if ("scriptsInEvents.js" === p || p.endsWith("/scriptsInEvents.js")) r = await this._MaybeGetCordovaScriptURL(r), await b(r);
        ...
        PostToRuntimeComponent(c, h, k, l, q) {
        ...
        _HasPermission(c) {
            ...
            return new Promise((k, l) => h.checkPermission(this._MapPermissionID(h, c), q => k(!!q.hasPermission), l))
        }
        _RequestPermission(c) {
            const h = this._GetPermissionAPI();
            return new Promise((k, l) => h.requestPermission(this._MapPermissionID(h, c), q => k(!!q.hasPermission), l))
        }
        ...
        _BinaryStringToBlob(c, h) {
            ...
            let l = k >> 2,
                ...
                r, w;
            for (w = r = 0; r < l; ++r) p[r] = c.charCodeAt(w++) | c.charCodeAt(w++) <<
                8 | c.charCodeAt(w++) << 16 | c.charCodeAt(w++) << 24;
            for (k &= 3; k--;) q[w] = c.charCodeAt(w), ++w;
            return new Blob([q], {
                type: h
            })
        }
...
"use strict"; {
    ...
    async function u(f, c, h) {
        ...
        if (l.hasAttribute("width") && l.hasAttribute("height")) {
            ...
            if (!q.includes("%") && !p.includes("%")) return await y(f)
        }
    ...
    self.C3_RasterSvgImageBlob = async function(f, c, h, k, l) {
        ...
        q.width = k;
        q.height = l;
        q.getContext("2d").drawImage(f, 0, 0, c, h);
        return q
    };
    let F = !1;
    ...
    function G() {
        ...
        return "textarea" === c ? !0 : "input" === c ? h.has(f.type.toLowerCase() || "text") : v(f)
    }
    d.AddDOMHandlerClass(class extends self.DOMHandler {
        ...
        Attach() {
            ...
            window.addEventListener("focusin", c => {
                C(c.target) && this._PostRuntimeEvent("keyboard-blur")
            });
        ...
        _TrySetTargetOrientation() {
            ...
            if (screen.orientation && screen.orientation.lock) screen.orientation.lock(f).catch(c => console.warn("[Construct 3] Failed to lock orientation: ", c));
            else try {
                let c = !1;
                screen.lockOrientation ? c = screen.lockOrientation(f) : screen.webkitLockOrientation ? c = screen.webkitLockOrientation(f) : screen.mozLockOrientation ? c = screen.mozLockOrientation(f) :
                    screen.msLockOrientation && (c = screen.msLockOrientation(f));
        ...
        _PlayPendingMedia() {
            ...
            if (!this._isSilent)
                for (const c of f)(f = c.play()) && f.catch(h => {
...
"use strict"; {
    ...
    self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMElementHandler {
        ...
        CreateElement(b, e) {
            let g;
            const m = e.type;
            "textarea" === m ? (g = document.createElement("textarea"), g.style.resize = "none") : (g = document.createElement("input"), g.type = m);
            g.style.position = "absolute";
            g.autocomplete = "off";
            g.addEventListener("touchstart", d);
            g.addEventListener("touchmove", d);
            ...
            g.addEventListener("dblclick", n => {
                n.stopPropagation();
                this._PostToRuntimeElementMaybeSync("dblclick", b)
            });
            g.addEventListener("input", () => this.PostToRuntimeElement("change",
                b, {
                    text: g.value
                }));
            e.id && (g.id = e.id);
...
"use strict"; {
    ...
    self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMElementHandler {
        constructor(a) {
            super(a, "list");
            this.AddDOMElementMessageHandler("set-selected-index", (b, e) => this._OnSetSelectedIndex(b, e.selectedIndex));
            this.AddDOMElementMessageHandler("add-item", (b, e) => this._OnAddItem(b, e));
            this.AddDOMElementMessageHandler("remove-item", (b, e) => this._OnRemoveItem(b, e));
            this.AddDOMElementMessageHandler("set-item", (b, e) => this._OnSetItem(b, e));
            this.AddDOMElementMessageHandler("clear",
                b => this._OnClear(b));
            this.AddDOMElementMessageHandler("load-state", (b, e) => this._OnLoadState(b, e))
        }
        CreateElement(a, b) {
            const e = b.isDropdown,
                g = b.isMultiSelect,
                m = b.items,
            ...
            n.multiple = g;
            e || (n.size = 2);
            for (const t of m) n.add(this._CreateOption(t));
            ...
            n.addEventListener("click", t => {
                t.stopPropagation();
                this._PostToRuntimeElementMaybeSync("click", a, this._GetSelectionState(n))
            });
            n.addEventListener("dblclick", t => {
                t.stopPropagation();
                this._PostToRuntimeElementMaybeSync("dblclick", a, this._GetSelectionState(n))
            });
            n.addEventListener("change", () => this.PostToRuntimeElement("change", a, this._GetSelectionState(n)));
            b.id && (n.id = b.id);
            this.UpdateState(n, b);
            return n
        }
        _CreateOption(a) {
        ...
        _GetSelectionState(a) {
            const b =
                a.selectedIndex,
                e = [];
            for (let g = 0, m = a.length; g < m; ++g) a.options[g].selected && e.push(g);
            return {
                selectedIndex: b,
                selectedIndices: e
            }
        ...
        UpdateState(a, b) {
            ...
            a.multiple = !!b.isMultiSelect
        }
        _OnSetSelectedIndex(a, b) {
            a.selectedIndex = b
        }
        _OnAddItem(a, b) {
            const e = b.index;
            b = this._CreateOption(b.text);
            0 > e ? a.add(b) : a.add(b, e)
        }
        _OnRemoveItem(a, b) {
            a.remove(b.index)
        }
        _OnSetItem(a, b) {
            a.options[b.index].text = b.text
        }
        _OnClear(a) {
            a.innerHTML = ""
        }
        _OnLoadState(a, b) {
            a.innerHTML = "";
            for (const e of b.items) a.add(this._CreateOption(e));
            a.selectedIndex = b.selectedIndex;
            for (const e of b.selectedIndices)
                if (b = a.options[e]) b.selected = !0
        }
    })
}
"use strict";
    ...
    self.AudioDOMHandler = class extends self.DOMHandler {
        ...
        async _Play(a) {
            const b = a.originalUrl,
                ...
                u = a.panning;
            let v = a.off;
            0 < v && !a.trueClock && (this._audioContext.getOutputTimestamp ? (a = this._audioContext.getOutputTimestamp(), v = v - a.performanceTime / 1E3 + a.contextTime) : v = v - performance.now() / 1E3 + this._audioContext.currentTime);
            this._lastPlayedTag = n;
            this._AddPendingTag(n);
            try {
                this._lastAudioInstance =
                    await this._GetAudioInstance(b, e, g, n, m), u ? (this._lastAudioInstance.SetPannerEnabled(!0), this._lastAudioInstance.SetPan(u.x, u.y, u.angle, u.innerAngle, u.outerAngle, u.outerGain), u.hasOwnProperty("uid") && this._lastAudioInstance.SetUID(u.uid)) : this._lastAudioInstance.SetPannerEnabled(!1), this._lastAudioInstance.Play(t, y, z, v)
            } catch (A) {
            ...
            if (3 !== b) {
                ...
                if (!n || 1 !== b)
                    if (n || 2 !== b) {
                        b = null;
                        try {
                            b = await this._GetAudioInstance(e, g, m, t, n)
                        } catch (v) {
...
"use strict"; {
    ...
    self.C3AudioFilterFX = class extends d {
        constructor(a, b, e, g, m, n, t) {
            super(a);
            this._type = "filter";
            this._params = [b, e, g, m, n, t];
...
"use strict"; {
    ...
    self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMElementHandler {
        ...
        CreateElement(a, b) {
            ...
            var g = e;
            b.isCheckbox ? (e.type = "checkbox", g = document.createElement("label"), g.appendChild(e), g.appendChild(document.createTextNode("")), g.style.fontFamily = "sans-serif", g.style.userSelect = "none", g.style.webkitUserSelect = "none", g.style.display = "inline-block", g.style.color = "black") : e.type = "button";
            g.style.position =
                "absolute";
            g.addEventListener("touchstart", d);
            g.addEventListener("touchmove", d);
        ...
        UpdateState(a,
            b) {
            const e = this._GetInputElem(a);
            e.checked = b.isChecked;
            e.disabled = !b.isEnabled;
            a.title = b.title;
            a === e ? e.value = b.text : a.lastChild.textContent = b.text
        }
    })
};