...
function UpdateCheck(isFirst) {
    return fetchWithBypass(OFFLINE_DATA_FILE, true).then(r => r.json()).then(data => {
        ...
        return caches.has(currentCacheName).then(cacheExists => {
            ...
            return GetMainPageUrl().then(mainPageUrl => {
                ...
                return CreateCacheFromFileList(currentCacheName, fileList, !isFirst).then(IsUpdatePending).then(isUpdatePending => {
                    if (isUpdatePending) {
...
self.addEventListener('fetch', event => {
    ...
    let responsePromise = GetAvailableCacheNames().then(availableCacheNames => {
        ...
        return Promise.resolve().then(() => {
            ...
            return caches.open(useCacheName).then(c => c.match(event.request)).then(response => response || fetch(event.request));
        });
    });
